### NP文章中trim时参数-illumina不要添加，否则adapter去不掉
###file_path
input_path=$1
file_path=`pwd`
...
fq2=${fq1/R1.fastq.gz/R2.fastq.gz}
    ...
    then
	~/software/anaconda3/envs/py36/bin/trim_galore -j 20 -o ${trimmedFastq_dir} $fq1 $fq2 --path_to_cutadapt ~/software/anaconda3/envs/py36/bin/cutadapt \
...
#step 4.1
### Making normalized bigWig files with reads adjusted by spike-in ###
mkdir -p ${bw_dir}
...
do
    ...
    then
        macs2 callpeak -t ${bam_file} -f BAMPE -n ${peak_dir}/peak_alone/"Broad_"$sample --nomodel --broad -g hs --broad-cutoff 0.01 -q 0.01 2>${peak_log}/"Broad_"${sample}/${sample}_alone.log
        bedtools intersect -a ${peak_dir}/peak_alone/"Broad_"${sample}_peaks.broadPeak -b $blacklist -f 0.25 -v > ${peak_dir}/peak_alone/"Broad_"${sample}_peaks.final.broadPeak &
	macs2 callpeak -t ${bam_file} -f BAMPE -n ${peak_dir}/peak_alone/"Regular_"$sample --nomodel -g hs -q 0.01 2>${peak_log}/"Regular_"${sample}/${sample}_alone.log
        bedtools intersect -a ${peak_dir}/peak_alone/"Regular_"${sample}_peaks.broadPeak -b $blacklist -f 0.25 -v > ${peak_dir}/peak_alone/"Regular_"${sample}_peaks.final.broadPeak &
    fi
done
wait
