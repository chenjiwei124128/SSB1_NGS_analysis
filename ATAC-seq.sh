...
#step 3.1
###Aligning to experimental genome 去除chrM reads#####
mkdir ${align_exp_dir}
...
if [ ! -s "${exp_bam_rmdup}/${sample}.rmdup.bam" ]
	...
	~/software/anaconda3/envs/py36/bin/alignmentSieve --numberOfProcessors 25 --ATACshift -b ${exp_bam_rmdup}/${sample}.rmdup_rmChrM.bam -o ${exp_bam_rmdup}/${sample}.rmdup_rmChrM_Shift.bam 
...
Ecoli_path=${rmdup_spike_log}
align_path=${alignexp_log_dir}

echo -e "sample\tALLREADS\tHg19_READS\tHg19_mapping_RATIO\tHg19_qc_READS\tHg19_qc_RATIO\tHg19_rmChrM_READS\tHg19_rmChrM_RATIO\tmtDNA_READS\tmtDNA_RATIO\tEcoli_qc_READS\tEcoli_qc_RATIO_intotal\tEcoli_qc_RATIO_inqc\tSCALEFACTOR\t" >${logs_dir}/scalefactor.txt
...
do
    ...
    Hg19_rmChrM_READS=$(cat $hg19_path/${sample}"_hg19.rmdup_rmChrM_Shift.sorted.stat"|grep "total (QC-passed reads"|cut -d " " -f 1)
    Hg19_rmChrM_RATIO=`printf "%.2f\n" $(echo "${Hg19_rmChrM_READS}/${ALLREADS}*100"|bc -l)`
    mtDNA_READS=$(samtools idxstats ${align_exp_dir}/${sample}"_hg19.bam" | grep 'chrM' | awk '{SUM += $3} END {print SUM}')
    mtDNA_RATIO=`printf "%.2f\n" $(echo "${mtDNA_READS}/${ALLREADS}*100"| bc -l)`
    Ecoli_qc_READS=$(cat ${Ecoli_path}/${sample}_Ecoli.rmdup.stat|grep "total (QC-passed reads"|cut -d "+" -f 1)
    QC_reads=$(echo "${Ecoli_qc_READS}+${Hg19_qc_READS}"|bc )
    Ecoli_qc_RATIO_intotal=`printf "%.2f\n" $(echo "${Ecoli_qc_READS}/${ALLREADS}*100"|bc -l)`
    Ecoli_qc_RATIO_inqc=`printf "%.2f\n" $(echo "${Ecoli_qc_READS}/${QC_reads}*100"|bc -l)`
    SCALEFACTOR=$(echo "1000000/${Ecoli_qc_READS}"|bc -l)
    echo -e $sample"\t"$ALLREADS"\t"$Hg19_READS"\t"$Hg19_mapping_RATIO"\t"$Hg19_qc_READS"\t"$Hg19_qc_RATIO"\t"$Hg19_rmChrM_READS"\t"$Hg19_rmChrM_RATIO"\t"$mtDNA_READS"\t"$mtDNA_RATIO"\t"$Ecoli_qc_READS"\t"$Ecoli_qc_RATIO_intotal"\t"$Ecoli_qc_RATIO_inqc"\t"$SCALEFACTOR >> ${logs_dir}/scalefactor.txt
done
...
bam_file=${exp_bam_rmdup}/${sample}_hg19.rmdup_rmChrM_Shift.sorted.bam
...
ls ${exp_bam_rmdup}/*_hg19.rmdup_rmChrM_Shift.sorted.bam | grep -v 'nput' | while read id;
do
    sample=`basename $id | sed 's/_hg19.rmdup_rmChrM_Shift.sorted.bam//g'`
    bam_file=${exp_bam_rmdup}/${sample}_hg19.rmdup_rmChrM_Shift.sorted.bam
    ...
    then
        macs2 callpeak -t ${bam_file} -f BAMPE -n ${peak_dir}/peak_alone/$sample --min-length 100 --nolambda -g hs --keep-dup all --nomodel 2>${peak_log}/${sample}/${sample}_alone.log
